...
No changes to scrape.py are needed.
"""
...
def build_payload(scraped: dict, products: dict, metadata: dict) -> list:
    ...
    for product_key, product_data in scraped.items():
        ...
        # Affiliate URLs live in products.json under each site key
        product_urls = {}
        ...
        for site_name, info in sites.items():
            ...
            # Skip — no valid price (includes "price_not_found")
            if price_value is None:
                continue

            # The affiliate URL from products.json (may be empty string)
            affiliate_url = product_urls.get(site_name, "").strip() or None
            product_url   = scraped_url.strip()

            # If we have no product URL at all, skip
...
def main():
    ...
    if not payload:
        print("⚠️  No valid price entries to push. Check scraped_data.json.")
        sys.exit(0)
